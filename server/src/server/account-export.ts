 ...
 * are intentionally excluded. Row objects come from node:sqlite (null prototype)
 * but are returned as JSON, so no RSC serialization concern.
 */
...
function rows(sql: string, ...params: ReadonlyArray<unknown>) {
  return database.prepare(sql).all(...params as never[]);
}

function one(sql: string, ...params: ReadonlyArray<unknown>) {
  return database.prepare(sql).get(...params as never[]) ?? null;
}
