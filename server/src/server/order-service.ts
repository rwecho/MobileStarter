...
import { paymentProviderForPlatform, storeKeyForPlatform } from './payment-providers';
...
export async function verifyPurchase(input: VerifyInput): Promise<OrderView> {
  ...
  // provider 是业务启用标识；真实适配器按上报平台分流（ios→apple / android→google）
  const provider = paymentProviderForPlatform(plan.provider, input.platform, input.environment);
