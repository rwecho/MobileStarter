...
// Point the SQLite singleton at a per-process temp file BEFORE importing any
// module that transitively loads the database (ESM evaluates dependencies first,
// so the import must be deferred until after this assignment).
process.env.MOBILEUI_DATABASE_PATH = join(tmpdir(), `mobileui-auth-${process.pid}.db`);
...
function latestVerificationCode(email: string): string | undefined {
...
test('sign-up persists consent, leaves email unverified, and issues a verification code', async () => {
  ...
  assert.equal(userRow(email).consent_version, '2026-07-29');
  assert.equal(userRow(email).email_verified, 0);
  assert.ok(latestVerificationCode(email));
});
...
test('repeated failed sign-ins lock the account and clear after cooldown', async () => {
  ...
  database.prepare('UPDATE sign_in_attempts SET locked_until = ? WHERE app_id = ? AND identifier = ?')
...
test('refresh token rotates and the previous token is revoked', async () => {
  ...
  const rotated = rotateRefreshToken(APP, first);
...
test('reusing a rotated refresh token revokes the whole family', async () => {
  ...
  const { refreshToken: second } = rotateRefreshToken(APP, first);
  assert.throws(
...
test('email verification rejects a wrong code, accepts the right one, and rejects reuse', async () => {
  ...
  assert.deepEqual(verifyEmail(APP, email, code), { verified: true });
  assert.equal(userRow(email).email_verified, 1);
  assert.throws(
